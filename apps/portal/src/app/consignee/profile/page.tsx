...
import { updateConsignee } from '@/services/consignee.service';
...
export default function ConsigneeProfilePage() {
  ...
  const consigneeInfo = fullInfo as Consignee;
  ...
  const [formData, setFormData] = useState({
    ...
    taxCode: consigneeInfo?.taxCode || '',
    certificate: consigneeInfo?.certificate || '',
    qrCode: consigneeInfo?.qrCode || ''
  });

  useEffect(() => {
    if (user && consigneeInfo) {
      setFormData({
        firstName: user?.firstName || '',
        lastName: user?.lastName || '',
        email: user?.email || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!consigneeInfo?.id) {
      toast({
        variant: 'destructive',
        title: t('toast.error'),
        description: t('toast.consigneeNotFound')
      });
      return;
    }
        ...
        <div className='flex items-center justify-between'>
          ...
          {!isEditing && (
            <div className='flex items-center space-x-2'>
              <Button
                variant='outline'
                onClick={() => router.push('/consignee/debt')}
              >
                <IconCreditCard className='mr-2 h-4 w-4' />
                {t('buttons.settleInvoice')}
              </Button>
                ...
                <div className='space-y-2'>
                  ...
                  {isEditing ? (
                    ...
                    <div className='flex items-center space-x-2 rounded-md border p-2'>
                      ...
                      <span>
                        {formData.contact || t('personalInformation.notSet')}
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div className='space-y-2'>
                  ...
                  {isEditing ? (
                    ...
                    <div className='flex items-center space-x-2 rounded-md border p-2'>
                      ...
                      <span>
                        {formData.address || t('personalInformation.notSet')}
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div className='space-y-2'>
                  ...
                  {isEditing ? (
                    ...
                    <div className='flex items-center space-x-2 rounded-md border p-2'>
                      ...
                      <span>
                        {formData.taxCode || t('personalInformation.notSet')}
                      </span>
                    </div>
                  )}
                </div>
