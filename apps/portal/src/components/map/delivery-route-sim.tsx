...
export default function DeliveryRouteSim({
  cargo,
  startAddress,
  endAddress,
  productName,
  ...
  useEffect(() => {
    ...
    let mounted = true;
    if (startAddress) {
      geocode(startAddress).then((p) => {
        if (!mounted) return;
        if (p) setFrom(p);
      });
    }
    if (endAddress) {
      geocode(endAddress).then((p) => {
        if (!mounted) return;
        if (p) setTo(p);
      });
    }
  ...
  useEffect(() => {
    ...
    fetchDeliveriesByOrderSchedule({ orderScheduleId: sid, page: 1, limit: 10 })
      .then((res) => {
        const d =
          Array.isArray(res?.data) && res.data.length > 0
            ? res.data[0]
            : undefined;
        if (d?.id) setAutoDeliveryId(String(d.id));
      })
      .catch(() => {});
