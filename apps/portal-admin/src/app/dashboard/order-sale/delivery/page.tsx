...
export default function DeliveryPage() {
  ...
  const stats = useMemo(() => {
    ...
    const totalDeliveries = page.deliveries.deliveries.length;
    const completedDeliveries = page.deliveries.deliveries.filter(
      (d) => d.status === 'completed'
    ).length;
    const activeDeliveries = page.deliveries.deliveries.filter(
      (d) => d.status === 'delivering' || d.status === 'scheduled'
    ).length;
    const todayDeliveries = page.deliveries.deliveries.filter((d) => {
  ...
  }, [page.schedules.allSchedules, page.deliveries.deliveries]);

  const isLoading =
    page.schedules.loading ||
    page.deliveries.loadingFetch ||
    page.trucks.loading ||
    page.deliveryStaffs.loading;

  return (
    <RouteGuard permission={Permission.MANAGE_SALE_DELIVERY}>
            ...
            <div className='grid flex-1 gap-3 overflow-hidden lg:grid-cols-[360px_1fr]'>
              ...
              <div className='overflow-hidden'>
                <PhaseDetailPane
                  ...
                  getDeliveryByPhaseId={page.deliveries.getDeliveryByPhaseId}
            ...
            <Card className='flex-1 overflow-hidden'>
              ...
              <CardContent className='overflow-auto px-4 py-2'>
                <div className='space-y-2'>
                  {page.deliveries.loadingFetch ? (
                  ...
                  ) : page.deliveries.deliveries.length === 0 ? (
                    ...
                    <div className='grid gap-2 md:grid-cols-2 xl:grid-cols-3'>
                      {page.deliveries.deliveries.map((delivery) => (
                        <Card key={delivery.id} className='overflow-hidden p-3'>
                          ...
                          <div className='mt-2 space-y-1 text-xs'>
                            {delivery.truck && (
                              <div className='flex items-center gap-1.5'>
                                ...
                                <span>{delivery.truck.licensePlate}</span>
                              </div>
                            )}
